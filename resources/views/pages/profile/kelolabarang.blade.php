      ...
      <div class="row no-gutters">
        ...
        <table width="100%" class="table table-hover table-striped table-dark">
          <thead>
              ...
              <th style="text-align: center" width="10%">Harga saat ini</th>
          ...
          @foreach($barangs as $barang)
            <tr id="barang{{$barang->id}}">
              ...
              <td style="text-align: center" id="brghargalelang{{$barang->id}}">Rp. {{number_format($barang->Lelang->harga,2,',','.')}}</td>
              <td style="text-align: center" id="brgstatus{{$barang->id}}">
                @if($barang->Lelang->status == "INACTIVE")
                  <input type="checkbox" id="tgl_lelang{{$barang->id}}" dataID="{{$barang->id}}" class="toggle_lelang" data-toggle="toggle;center" data-on="ON GOING" data-off="INACTIVE" data-style="slow" data-offstyle="danger">
                @elseif($barang->Lelang->status == "ON GOING")
                  <input type="checkbox" id="tgl_lelang{{$barang->id}}" dataID="{{$barang->id}}" class="toggle_lelang" checked data-toggle="toggle" data-on="ON GOING" data-off="INACTIVE" data-style="slow" data-offstyle="danger">
                @elseif($barang->Lelang->status == "ENDED")

                @endif
                <br>
                <hr>
                @if($barang->Lelang->status == "ENDED")
                  <button dataID="{{$barang->id}}" class="btn btn-danger btn_end_lelang" disabled>SELESAIKAN</button>
                @else
                  <button dataID="{{$barang->id}}" class="btn btn-danger btn_end_lelang">SELESAIKAN</button>
                @endif
              </td>
              <td style="text-align: center" id="brgdurasi{{$barang->id}}">{{$barang->Lelang->durasi}} hari</td>
              <td style="text-align: left" id="brgket{{$barang->id}}">{{$barang->keterangan_barang}}</td>
